export default function Schedules() {
    return (
        <div className="container">
          ...
          <p>There are three commands to handle schedules: start, running and stop. ?start is used to start a
          ...
          <p>Now, I promise this is simpler than it looks. The first argument is the name of the schedule, which
            ...
            point, the interval and the amount of times the schedule will run. The rest of the arguments are
            equivalent to a command's arguments, but used by the schedule. In this case the arguments are the
            message that will be sent by the bot.</p>
          ...
          <p>And here's where it gets a bit wackier than Dyno. If you want to do something like 1 minute 30
            seconds, you can do 90s, or you can just do 1m30s. Or even 1s1s1s1s...1s with 90 of those "1s".
          ...
          <p>To make an interval, just add a "-" to the end of an amount of time. When the only time argument is
            an interval, it executes that amount of time from now, and then that amount of time from then, etc.
            In the case of the command above, it executes 1 hour from now, then 2 hours from now, 3 hours from
            now, etc. Of course, you can make intervals like 13m2s- too.</p>
          ...
          <p>With the first time argument being a time amount and the second being an interval, it executes after
            the first amount of time from now, and then on the interval in the second argument. In this case, the
            bot says "wazzup in the future" starting 3 days from now, and then every 2 hours.</p>
          ...
          <p>It works as expected with the time interval, but will stop running after 3 times. Of course you can
            do any number of times after the x</p>
          ...
          <p>One last thing, "now" is similar to a time amount like 0s or 0m. You can't have an interval now-,
            but you can use now like</p>
